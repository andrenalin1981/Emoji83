		...
		if (isSkinUnicode(stringChar) && charIndex - 1 >= 0) {
			...
			if (firstSkinChar == 0xD83C) {
     			...
     			if (charIndex - 2 >= 0) {
     				...
     				BOOL Short = checkEmoji == 0x261D || checkEmoji == 0x270A || checkEmoji == 0x270B || checkEmoji == 0x270C;
     				if (Short) {
     					BOOL dingbat = (checkEmoji == 0x261D || checkEmoji == 0x270C);
     					NSUInteger shortLength = (dingbat && (charIndex + 1 < length)) ? 4 : 3;
     					NSRange skinDingbatRange = NSMakeRange(charIndex - 2, shortLength);
						...
						if (charIndex - 3 >= 0) {
     						...
     						if ((checkEmoji2 == 0xD83C || checkEmoji2 == 0xD83D) && !Short) {
     							BOOL isDiversed = NO;
